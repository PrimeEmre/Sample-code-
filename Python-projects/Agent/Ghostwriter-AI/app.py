...
TICKER_CORRECTIONS = {
    ...
    "TESLA": "TSLA",   "NVIDIA": "NVDA",    "BITCOIN": "BTC-USD",
    "ETHEREUM": "ETH-USD", "SOLANA": "SOL-USD", "BTC": "BTC-USD",
...
def kickoff_crew(topic: str, tone: str, length: str) -> str:
    payload = {
        "inputs": {
            ...
            "length": LENGTH_WORDS.get(length, "1200-1800 words"),
            "today":  str(date.today()),
        }
    }
    print(f"DEBUG: Calling URL: {CREWAI_KICKOFF_URL}")
    print(f"DEBUG: Headers: {get_crew_headers()}")
    ...
    if not kid:
        raise ValueError(f"No kickoff_id in response: {data}")
    return kid
...
def run_analysis(ticker: str) -> dict:
    ...
    news          = web_search(f"{ticker} stock news {date.today().year}")
    messages = [
        {"role": "system", "content": "You are a senior financial analyst. Give a clear BUY/HOLD/SELL verdict."},
        {"role": "user",   "content": f"Analyze {ticker}. Price: ${current_price}. {fear_greed}. News: {news}"}
    ]
    return {"ticker": ticker, "price": current_price, "fear_greed": fear_greed, "report": ollama_chat(messages)}
...
def stock():
    data   = request.get_json()
    ticker = TICKER_CORRECTIONS.get(data.get("ticker","").strip().upper(), data.get("ticker","").upper())
    if not ticker:
        return jsonify({"error": "No ticker provided."}), 400
    try:
        return jsonify(run_analysis(ticker))
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def speak():
    ...
    try:
        payload = {
            ...
            "voice":       {"languageCode": "en-US", "name": data.get("voice_id", "en-US-Studio-O")},
            "audioConfig": {"audioEncoding": "MP3"}
        }
        resp       = requests.post(GOOGLE_TTS_URL, json=payload, timeout=30)
        resp.raise_for_status()
